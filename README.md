...
**Finding your `.verse` files.** Sortilege auto-detects your project's real directory from one of your own scanned assets' on-disk path (`unreal.SystemLibrary.get_system_path()`), then searches that folder's `.verse` files. This is deliberately NOT `unreal.Paths.project_dir()` -- a live diagnostic proved that call resolves to the Fortnite ENGINE directory inside UEFN, not your project, which used to make this feature silently scan the wrong folder and find (and fix) nothing, even with a genuine reference sitting right there in your own `.verse` code. Every preview and report always prints "Verse fixup: N .verse file(s) found under X" so you can see exactly which directory it used; if it ever looks wrong for your setup, set `CONFIG["VERSE_SEARCH_DIR"]` to your project's folder (the one containing your `.uefnproject`) and that override always wins, no auto-detection involved. Honesty note: `unreal.SystemLibrary.get_system_path()`'s exact behavior on asset objects has not been confirmed against a live UEFN session as part of this fix (only against the test mock), and the override above is always there as a manual escape hatch.

Those are the only two directories Sortilege will ever search: the one it derived from your own assets, and the one you set by hand. It deliberately will not fall back to `unreal.Paths.project_dir()` even when nothing else resolves. Having `.verse` files under it is evidence that a folder holds Verse source, not evidence of *whose* Verse source it is, so a project with no Verse code of its own would otherwise fall through to the Fortnite engine directory, find Fortnite's own `.verse` files, and edit a file inside your Fortnite install. If Sortilege cannot work out a directory at all, it says so in the preview instead of guessing, and an `apply` refuses to move anything when your dry run DID find `.verse` files that the real run can no longer see. Nothing gets moved in that case, so nothing is left half-done: set `CONFIG["VERSE_SEARCH_DIR"]` and run again, or set `CONFIG["FIX_VERSE_REFERENCES"]` to `False` to sort without the Verse fixup and update your Verse references by hand.

A few things worth knowing:

- **A root-level asset name (no folder qualification, just `T_Hex`) is always called out separately in the preview.** By default it is listed as "skipped (bare name - fix manually)" and left alone; if you turn the setting below on, it is rewritten and flagged "(bare name - review)" instead. Either way it is a plain word with nothing to distinguish it from an unrelated identifier that merely happens to match, so it is worth a look in the preview before you apply, even though the same whole-token matching still protects it from a partial-word false hit.
- **`CONFIG["FIX_VERSE_BARE_NAMES"]` (default `False`) controls whether a bare name actually gets rewritten.** Left `False`, Sortilege still rewrites every qualified (dotted) reference automatically -- the common, checkable case -- but SKIPS a bare-name rewrite, listing it in the preview and report under "skipped (bare name - fix manually): N" so you know exactly which ones to update by hand. The reason this is the default: a bare name is just a word, and nothing in the file tells Sortilege whether `Sphere` is your moved mesh or a variable you named `Sphere`. Set it to `True` if your project's bare references are worth the automatic rewrite; they are always flagged "(bare name - review)" in the preview when you do.
- **Only real code is ever rewritten.** A match inside a comment or inside a `"quoted string"` is left exactly as it was, and so is a name that is being declared rather than referenced (`Sphere : int = 9` declares something new that merely happens to share the name; rewriting it would produce a file that no longer compiles). This applies to the preview and to the actual file on disk alike, so what the preview shows you is what you get. One limitation worth knowing: Sortilege reads one line at a time, so it does not track a multi-line `<# ... #>` block comment. That can only ever make it rewrite less than it might, never more.
